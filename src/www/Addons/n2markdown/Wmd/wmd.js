	...
	util.prompt = function(text, defaultInputText, makeLinkMarkdown) {
		...
		var close = function(isCancel) {
			...
			if (isCancel) {
				...
				// FIX FOR N2 (check for url starting with /)
				if (text.indexOf("/") != 0) {
		...
		var createDialog = function() {
			...
			dialog.style.border = "1px solid #999999"; // FIX FOR N2
			dialog.style.backgroundColor = "#F5F5F5"; // FIX FOR N2
			...
			input.id = "wmddialoginput"; // ADDED FOR N2
			style = input.style;
			style.display = "block";
			style.width = "80%";
			...
			// ---- ADDED FOR N2
			var n2Chooser = doc.createElement("input");
			n2Chooser.type = "button";
			n2Chooser.value = "...";
			window.onFileSelected = function(url) {
				input.value = url;
			}
			window.srcField = "xyz";
			n2ManagerUrl = "/Edit/FileManagement/default.aspx?availableModes=All&tbid=wmddialoginput&destinationType=input&selectedUrl=";
			n2Chooser.onclick = function() {
				window.open(n2ManagerUrl, "n2chooser", "width=400,height=500,status=0,toolbar=0");
			}

			style = n2Chooser.style;
			style.margin = "10px";
			style.display = "inline";
			style.width = "35px";

			form.appendChild(n2Chooser);
			// ---- END ADDED FOR N2
